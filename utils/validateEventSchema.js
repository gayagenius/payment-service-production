...
function loadPaymentEventSchemas() {
  const schemas = {
    ...
    payment_completed: {
      ...
      "type": "object",
      "required": ["eventType", "timestamp", "paymentId", "orderId", "userId", "amount", "currency", "status", "correlationId"],
      "properties": {
    ...
    payment_failed: {
      ...
      "type": "object",
      "required": ["eventType", "timestamp", "paymentId", "orderId", "userId", "amount", "currency", "status", "correlationId", "error"],
      "properties": {
    ...
    refund_processed: {
      ...
      "type": "object",
      "required": ["eventType", "timestamp", "refundId", "paymentId", "orderId", "userId", "amount", "currency", "status", "correlationId"],
      "properties": {
...
export function initializeSchemaValidation() {
  try {
    ...
    console.log(`Initialized schema validation for ${schemaCache.size} event types`);
    return true;
  } catch (error) {
...
export class SchemaMigrator {
  ...
  migrate(payload, fromVersion, toVersion) {
    ...
    if (!migrationFn) {
      throw new Error(`No migration available from ${fromVersion} to ${toVersion}`);
    }

    try {
      return migrationFn(payload);
    } catch (error) {
      throw new Error(`Migration failed from ${fromVersion} to ${toVersion}: ${error.message}`);
    }
  }
}
