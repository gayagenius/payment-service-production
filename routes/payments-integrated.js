...
import {
  processPayment,
  createPaymentMethodForGateway,
} from "../services/paymentProcessor.js";
...
router.get("/", async (req, res) => {
  try {
    ...
    const limitNum = Math.min(
      parseInt(limit) || API_CONFIG.DEFAULT_PAGINATION_LIMIT,
      API_CONFIG.MAX_PAGINATION_LIMIT
    );
    const offsetNum = Math.max(
      parseInt(offset) || API_CONFIG.DEFAULT_PAGINATION_OFFSET,
      0
    );
    ...
    query += ` ORDER BY p.created_at DESC LIMIT $${paramCount + 1} OFFSET $${
      paramCount + 2
    }`;
...
router.post("/", async (req, res) => {
  try {
    ...
    const finalIdempotencyKey =
      idempotencyKey || `ref_${user_id}_${order_id}_${Date.now()}`;
    ...
    if (gatewayResult.success) {
      ...
      try {
        await publishPaymentEvent("payment_processed", {
          paymentId: paymentResult.payment_id, // <-- camelCase expected by publisher
          orderId: order_id,
          userId: user_id,
          amount,
    ...
    const responseData = {
      ...
      gatewayResponse: gatewayResult.success
